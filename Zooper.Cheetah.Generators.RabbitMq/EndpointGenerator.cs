...
// Allows init-only properties in records for .NET Standard 2.0
...
{
	/// <summary>
	/// Helper class for diagnostic reporting
	/// </summary>
	internal record DiagnosticInfo
	{
		public Diagnostic Diagnostic { get; init; } = null!;
	...
	public sealed class EndpointGenerator : IIncrementalGenerator
	{
		...
		public void Initialize(IncrementalGeneratorInitializationContext context)
		{
			...
			IncrementalValuesProvider<(Diagnostic, bool)> diagnostics = context.SyntaxProvider
				.CreateSyntaxProvider(
					predicate: (_, _) => true,
					transform: (_, _) => (
						Diagnostic.Create(ExecutionInfo, Location.None, "Initializing EndpointGenerator"),
						false
					))
				.Where(d => !d.Item2);

			context.RegisterSourceOutput(diagnostics, (spc, d) =>
				spc.ReportDiagnostic(d.Item1));
				...
				.CreateSyntaxProvider(
					predicate: (node, _) =>
					{
						...
						if (node is InterfaceDeclarationSyntax intf)
						{
							...
							return intf.Identifier.ToString().Contains("Integration") ||
								   intf.BaseList?.Types.Any(t => t.ToString().Contains("Integration")) == true;
						}
						return false;
					},
					transform: (ctx, ct) =>
					{
						...
						if (symbol == null) return new InterfaceInfo();

						var info = new InterfaceInfo { InterfaceName = symbol.ToDisplayString() };

						// Is this an integration event interface?
						if (!info.InterfaceName.Contains("IIntegrationEvent")) return info;
			...
			SourceProductionContext context)
		{
			...
			foreach (var consumer in consumers.Where(c => c != null))
			{
				...
				// If not matched with interface events, try channel events
				if (!matched)
				{
					foreach (var channelEvent in channelEvents.Where(c => c != null))
					{
						context.ReportDiagnostic(Diagnostic.Create(ExecutionInfo, Location.None,
							$"Checking {consumer.MessageType} against channel event {channelEvent!.TypeName}"));

						if (consumer.MessageType == channelEvent.TypeName ||
							consumer.MessageType.EndsWith("." + channelEvent.TypeName.Split('.').Last()))
						{
							endpointInfos.Add(new EndpointInfo
							{
								EventName = channelEvent.EventName,
								ConsumerType = consumer.ConsumerType,
								MessageType = consumer.MessageType
							});
							...
							context.ReportDiagnostic(Diagnostic.Create(
								ExecutionInfo,
								Location.None,
								$"Mapped consumer {consumer.ConsumerType} to channel event {channelEvent.EventName}"));
		...
		{
			sourceBuilder.AppendLine($"    public static void {MethodName}(this IRabbitMqBusFactoryConfigurator cfg, IBusRegistrationContext context, string serviceName = \"{DefaultServiceName}\")");
			sourceBuilder.AppendLine("    {");
